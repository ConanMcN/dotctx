...
1. **Initialize** — `dotctx init --scan` creates the `.ctx/` directory, installs the `/ctx-setup` skill, and sets up editor hooks
...
A `UserPromptSubmit` hook is installed at `.claude/hooks/dotctx-preflight.sh`. On every prompt, it runs `dotctx preflight` with your prompt text and injects the result as context. You get landmine warnings, relevant decisions, and ripple effects automatically — no need to remember to run preflight.

The hook is registered in `.claude/settings.json` with a 10-second timeout. It always exits 0 and never blocks your prompt.
...
After running `dotctx init`, the `/ctx-setup` slash command is also installed for Claude Code. Use it to scan your codebase and auto-populate `.ctx/` files.
